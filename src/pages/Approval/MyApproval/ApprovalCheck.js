...
class ApprovalCheck extends PureComponent {
  ...
  componentDidMount(){
    ...
    const initData = this.props.location.state.query
        ...
        if(res.resData && res.resData.length){
          ...
          res.resData.map((item)=>{
            for(let key in item){
              //发起人
              if(key === 'AUDITOR'){
                person = JSON.parse(item[key]).name
              }
  ...
  aggreeBack = ()=>{
    ...
    const obj = {
      reqData:{
        billid:initData.billId?Number(initData.billId):null,
        audittype:initData.type?initData.type:'',
        processinstanceid:initData.processInstanceId?initData.processInstanceId:'',
      }
    }
    dispatch({
