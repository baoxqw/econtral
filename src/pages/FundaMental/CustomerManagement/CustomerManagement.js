...
class CustomerManagement extends PureComponent {
  ...
  handleBussiness = () => {
    router.push({pathname:'/fundamental/customermanagement/add',record:{addid:this.state.id,title:this.state.title}});
  };
  componentDidMount(){
    const { dispatch} = this.props;
    //  树的数据
  ...
  handleUpdate = (flag,record) => {
    router.push({pathname:'/fundamental/customermanagement/update',record})
  }
  //取消
  handleReset = () => {
    ...
    return (
      <PageHeaderWrapper>
        <div style={{display:'flex'}}>
          ...
          <Card title="客户档案" style={{ width:'75%',boxSizing:'border-box',overflow:'hodden' }} bordered={false}>
            <Form onSubmit={this.handleSearch} layout="inline">
              <Row gutter={{xs: 24, sm: 24, md: 24 }}>
                <Col md={24} sm={24}>
                  <FormItem label='客户编码'>
                    {getFieldDecorator('code')(<Input placeholder='客户编码' />)}
                  </FormItem>
