...
@Form.create()
class ContactManagementUpdate extends PureComponent {
  state = {
    dataList:{},
  ...
  onsubmit = (onOk)=>{
    ...
    form.validateFields((err,values)=>{
      ...
      /*if(!values.billcode){
        return message.error("合同编码不能为空")
      }
      if(!values.billname){
        return message.error("合同编码不能为空")
      }
      if(!values.type){
        return message.error("合同类型不能为空")
      }
      if(!values.version){
        return message.error("版本号不能为空")
      }
      if(!values.signdate){
        return message.error("签约日期不能为空")
      }
      if(!values.signplace){
        return message.error("签约地址不能为空")
      }
      if(!values.planValidateTime){
        return message.error("计划生效日期不能为空")
      }
      if(!values.planTeminateTime){
        return message.error("计划终止日期不能为空")
      }
      if(!this.state.selectedClientRowKeys.length){
        return message.error("客户不能为空")
      }
      if(!this.state.departmentId){
        return message.error("经办部门不能为空")
      }
      if(!this.state.selectedManagerRowKeys.length){
        return message.error("经办人员不能为空")
      }
      if(!this.state.contractmny){
        return message.error("合同金额不能为空")
      }
      if(!values.status){
        return message.error("合同状态不能为空")
      }
      if(!values.additionalcharges){
        return message.error("额外费用不能为空")
      }
      if(!values.eca){
        return message.error("有效合同额不能为空")
      }*/
      const obj = {
        ...
        signplace:values.signplace,
        planValidateTime:values.planValidateTime.format('YYYY-MM-DD'),
        planTeminateTime:values.planTeminateTime.format('YYYY-MM-DD'),
        custId:this.state.selectedClientRowKeys[0],
        deptId:this.state.departmentId,
        operatorId:this.state.selectedManagerRowKeys[0],
        contractmny:values.contractmny?Number(values.contractmny):null,
        status:values.status,
        vdef1:values.vdef1,
        additionalcharges:values.additionalcharges?Number(values.additionalcharges):null,
        eca:values.eca?Number(values.eca):null,
        memo:values.memo,
      };
      console.log("obj",obj)
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data.record.id !== this.props.data.record.id){
      ...
      const custId = record.custId; //客户id
      const custName = record.custname;

      const deptId = record.deptId; //部门id
      const deptName = record.deptname;

      const operatorId = record.operatorId; //制单人
      ...
      this.setState({
        ...
        selectedClientRowKeys:[custId],
        SelectClientValue:custName,

        selectedManagerRowKeys:[operatorId],
        SelectManagerValue:operatorName,
            ...
            <Col lg={6} md={12} sm={24}>
              <Form.Item label="合同编码">
                {getFieldDecorator('billcode',{
                  rules: [{required: true}],
                  initialValue:dataList.billcode
            ...
            <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
              <Form.Item label="合同名称">
                {getFieldDecorator('billname',{
                  ...
                  initialValue:dataList.billname
                })(
            ...
            <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
              <Form.Item label="合同类型">
                {getFieldDecorator('type', {
                  ...
                  initialValue:dataList.type
                })(<Input placeholder="合同类型" />)}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col lg={6} md={12} sm={24}>
              <Form.Item label="版本号">
                {getFieldDecorator('version',{
                  // rules: [{required: true}],
                  initialValue:dataList.version
                })(<Input placeholder="版本号" disabled type={"Number"}/>)}
              </Form.Item>
            </Col>
            <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
              <Form.Item label="签约日期">
                {getFieldDecorator('signdate', {
                  ...
                  initialValue:dataList.signdate?moment(dataList.signdate):null
                })(<DatePicker  placeholder="请选择签约日期" style={{ width: '100%' }}/>)}
              </Form.Item>
            </Col>
            <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
              <Form.Item label="合同所在地址">
                {getFieldDecorator('signplace', {
                  ...
                  initialValue:dataList.signplace
                })(<Input placeholder={"请输入合同所在地址"}/> )}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col lg={6} md={12} sm={24}>
              <Form.Item label="计划生效日期">
                {getFieldDecorator('planValidateTime',{
                  rules: [{required: true}],
                  initialValue:dataList.planValidateTime?moment(dataList.planValidateTime):null
                })(<DatePicker  placeholder="请选择计划生效日期" style={{ width: '100%' }}/>)}
              </Form.Item>
            </Col>
            <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
              <Form.Item label="计划终止日期">
                {getFieldDecorator('planTeminateTime',{
                  rules: [{required: true}],
                  initialValue:dataList.planTeminateTime?moment(dataList.planTeminateTime):null
                })(<DatePicker  placeholder="请选择计划终止日期" style={{ width: '100%' }}/>)}
              </Form.Item>
            </Col>
            <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
              <Form.Item label="合同所在省份">
                {getFieldDecorator('vdef1', {
                  rules: [
                    {
                      required: true,
                      message:'合同所在省份'
                  ...
                  initialValue:dataList.vdef1?dataList.vdef1:'',
                })(<Select
                  style={{ width:'100%'}}
                  placeholder={'请选择省份'}
          ...
          <Row gutter={16}>
            ...
            <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
              <Form.Item label="合同金额">
                {getFieldDecorator('contractmny',{
                  rules: [{required: true}],
                  initialValue:dataList.contractmny
                })(<Input placeholder="合同金额" type="Number"/>)}
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col lg={6} md={12} sm={24}>
              <Form.Item label="合同状态">
                {getFieldDecorator('status', {
                  ...
                  initialValue:dataList.status
                })(<Input placeholder="合同状态" disabled/> )}
              </Form.Item>
            </Col>
            <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
              <Form.Item label="额外费用">
                {getFieldDecorator('additionalcharges',{
                  initialValue:dataList.additionalcharges
                })(<Input placeholder={'请输入额外费用'} type="Number"/>)}
              </Form.Item>
            </Col>
            <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
              <Form.Item label="有效合同额">
                {getFieldDecorator('eca',{
                  rules: [
                    {
                      required: true,
                    }
                  ],
                  initialValue:dataList.eca
...
export default ContactManagementUpdate;
