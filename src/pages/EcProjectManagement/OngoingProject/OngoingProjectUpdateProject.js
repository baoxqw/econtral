...
import Tabled from 'braft-extensions/dist/table';
import { connect } from 'dva';
import FooterToolbar from '@/components/FooterToolbar';
import PageHeaderWrapper from '@/components/PageHeaderWrapper';
import FinanceTableForm from '../../FundaMental/CorpAdmin/FinanceTableForm';
import ShareholderTableForm from '../../FundaMental/CorpAdmin/ShareholderTableForm';
import FinancingTableForm from '../../FundaMental/CorpAdmin/FinancingTableForm';
import storage from '@/utils/storage';
import LicenserouterTableForm from './LicenseTableForm';
import TeamTableForm from '../../FundaMental/CorpAdmin/TeamTableForm';

import DistributionPlanTableForm from '../../FundaMental/CorpAdmin/DistributionPlanTableForm';
...
@connect(({ fundproject, loading }) => ({
  fundproject,
  submitting: loading.effects['fundproject/fetchongoingupdate'],
  data:fundproject.detail,//详细信息
  shareholder:fundproject.shareholder,//股东结构
  historical:fundproject.history,//融资历史
  distribution:fundproject.distribution,//分配方案
  team:fundproject.team,//团队
  finance:fundproject.finance,//公司财务
  license:fundproject.license,//牌照
...
@Form.create()
class OngoingProjectUpdateProjectForms extends PureComponent {
  state = {
    width: '100%',
    ...
    istoken:false,
    cityed:false,
    projectFinanceList:[], //公司财务
    projectShareholderList:[], //股东结构
    projectHistoryList:[], //融资历史
    projectDistributionList:[], //分配方案
    projectTeamList:[], //团队
    dataList:[],
    lp_id:null,
    project_id:null,
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: 'memo',
      render:(item,record)=>{
        return < a target="_blank" href={`https://www.leapingtech.net/nien-0.0.1-SNAPSHOT${record.path}/${record.name}`} download>{item}</ a>
      }
    },
    {
      title: '上传人',
  ...
  handleDelete = (record)=>{
    ...
    dispatch({
      type: 'fundproject/attachmentdel',
      payload: {
        id: record.id,
        pageIndex
      },
  ...
  handleDelete2 = (record)=>{
    const { dispatch } = this.props;
    const { pageIndexend } = this.state;
    dispatch({
      type: 'fundproject/attachmentdeldue',
      payload: {
        id: record.id,
        pageIndex:pageIndexend
      },
      callback:(res)=>{
        const ob = {
          reqData:{
  ...
  handleUpload = e => {
    ...
    this.props.form.validateFields((err, values) => {
      console.log('备注信息：',values)
      mem = values.relamemo;
      this.setState({mem:values.relamemo})
    });
    if(!mem){
    ...
    fileList.forEach((file) => {
      ...
      formData.append('parentpath', 'ir');
      formData.append('status', 'IR');
...
//尽职调查上传
  handleUploadend= e => {
    ...
    const { dispatch,form} = this.props;
    const { fileListend } = this.state;
    const user = storage.get("userinfo");
    const corp_id = user.corp.id;
    var askmemo = '';
    this.props.form.validateFields((err, values) => {
      askmemo = values.askmemo;
    ...
    fileListend.forEach((file) => {
      formDatad.append('files[]', file);
      formDatad.append('type', 'business');
      formDatad.append('parentpath', 'dd');
      formDatad.append('status', 'DD');
      formDatad.append('corp_id', corp_id);
      formDatad.append('project_id', this.state.project_id);
      formDatad.append('memo',askmemo);
      formDatad.append('project_status', this.state.project_status);
    });
    ...
    dispatch({
      type: 'fundproject/submitProjectAddFormend',
      payload: formDatad,
      callback: () => {
        message.success('上传成功')
        form.resetFields();
        ...
        const st = {
          reqData:{
            bill_id:this.state.project_id,
            type:'dd',
            pageIndex:this.state.pageIndexend,
            pageSize:10
          }
        }
        dispatch({
  ...
  handleCountryChange = selectedItems =>{
    if(selectedItems === '0'){
      this.setState({cityed:true})
    }else{
      this.setState({cityed:false})
    }
  }
  ...
  componentDidMount(){
    ...
    const { dispatch,data,htmlvalue} = this.props;
    console.log('上一页数据：',this.props)
    if(this.props.location.state){
      const projectStatus = this.props.location.state.query.query.project_status
      this.setState({projectStatus})
      if(projectStatus =='POST'){
    ...
    //
    if(this.props.location.state){
      const dataProps = this.props.location.state.query;
      const project_id = dataProps.query.project_id;
      const project_status = dataProps.query.project_status;
      const id = dataProps.query.id;
      this.setState({
        dataProps,
        valid :id,
        project_id,
      ...
      dispatch({
        type: 'fundproject/fetchongoingupdate',
        payload:str,
        callback:(res)=>{
          ...
          let istoken = res.projectDetail.istoken || false;
          if(istoken === 1){
            istoken = true
          }
          if(istoken === 0){
          ...
          const projectFinanceList = res.projectFinanceList.map((item,i)=>{
            item.key = i;
            return item
          }); //财务
          const projectShareholderList = res.projectShareholderList.map((item,i)=>{
            item.key = i;
            return item
          }); //股东设置
          const projectHistoryList = res.projectHistoryList.map((item,i)=>{
            item.key = i;
            return item
          }); //融资历史
          const projectDistributionList = res.projectDistributionList.map((item,i)=>{
            item.key = i;
            return item
          }); //分配方案
          const projectTeamList = res.projectTeamList.map((item,i)=>{
            item.key = i;
            return item
          }); //团队
          ...
          this.setState({
            initSource:res,
            detailSource:res.projectDetail,
            istoken,
  ...
  validate = ()=> {
    ...
    const corpId = userinfo.corpId;//公司id
    const id = this.state.dataProps.query.project_id;//项目明细id
    form.validateFields((err, fieldsValue) => {

      const formData = form.getFieldsValue();
      if(!fieldsValue.project_name){
        return
      }
      ...
      if(!fieldsValue.maininvestor){
        return
      }
      if(!this.state.lp_id){
        message.error("请选择投资主体");
        return
      }
      if(!fieldsValue.contactperson){
        return
      }
      ...
      if(!fieldsValue.subscriptionamount){
        message.error("认购金额不能为空");
        return
      }
      if(!fieldsValue.noofshare){
      ...
      if(!fieldsValue.preinvestor){
        message.error("上轮投资人不能为空");
        return
      }
      if(!fieldsValue.investor){
        message.error("本轮投资人不能为空");
        return
      }
      if(!fieldsValue.otherinvestors){
        message.error("其他投资人不能为空");
        return
      }
      if(this.state.investtype === '1'){ //股权投资
        if(!fieldsValue.stocktype){
          message.error("股权类型不能为空");
          return
        }
        if(!fieldsValue.investtool){
          message.error("投资工具不能为空");
          return
        }
        if(!fieldsValue.pricepershare){
          message.error("每股价格不能为空");
          return
        }
        if(!fieldsValue.parvalue){
          message.error("票面价格不能为空");
          return
        }
        if(!fieldsValue.noofcapital){
          message.error("总资本不能为空");
          return
        }
        if(!/^(\d+)((?:\.\d+)?)$/.test(fieldsValue.shareholdingratio)){
          message.error("持股比例不合法");
          return
        }
        if(!/^(\d+)((?:\.\d+)?)$/.test(fieldsValue.couponrate)){
          message.error("折扣率不合法");
          return
        }
        if(!fieldsValue.yieldtomaturity){
          message.error("到期收益率不能为空");
          return
        }
        if(!fieldsValue.conversionprice){
          message.error("转换价格不能为空");
          return
        }
      }
      if(this.state.investtype === '2'){
        if(!fieldsValue.ipoexchange){
          message.error("发行平台不能为空");
          return
        }
        if(!/^((ht|f)tps?:)\/\/[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]/g.test(fieldsValue.www)){
          message.error("网址不合法");
          return
        }
        if(!fieldsValue.tag){
        ...
        if(!fieldsValue.tokentype){
          message.error("代币类型不能为空");
          return
        }
        if(!fieldsValue.hardcap){
          message.error("硬顶不能为空");
          return
        }
        if(!fieldsValue.softcap){
          message.error("软顶不能为空");
          return
        }
        if(!fieldsValue.totalsupply){
          message.error("总供给不能为空");
          return
        }
        if(!fieldsValue.forsalerate){
          message.error("可售比率不能为空");
          return
        }
        if(!fieldsValue.lockup){
          message.error("锁定数量不能为空");
          return
        }
        if(!fieldsValue.valuation){
          message.error("本轮估值不能为空");
          return
        }
        if(!fieldsValue.prevaluation){
          message.error("上轮估值不能为空");
          return
        }
        if(!fieldsValue.premium){
          message.error("本轮估值溢价不能为空");
          return
        }
        if(!fieldsValue.forsale){
        ...
        if(!/^(\d+)((?:\.\d+)?)$/.test(fieldsValue.discount)){
          message.error("折扣不合法");
          return
        }
      }
      ...
      const investplan = {
        ...
        corp_id:corpId,
        investtype:fieldsValue.investtype?Number(fieldsValue.investtype):null, // 投资类型
        stocktype:fieldsValue.stocktype?Number(fieldsValue.stocktype):null,  //股权类型
        investtool:fieldsValue.investtool,  // 投资工具
        subscriptionamount:fieldsValue.subscriptionamount, // 认购金额
        noofshare:fieldsValue.noofshare?Number(fieldsValue.noofshare):null,   //股数
        currency: fieldsValue.currency, //币种
        pricepershare: fieldsValue.pricepershare?Number(fieldsValue.pricepershare):null, //每股价格
        parvalue:fieldsValue.parvalue?Number(fieldsValue.parvalue):null, //票面价格
        noofcapital:fieldsValue.noofcapital?Number(fieldsValue.noofcapital):null, //总资本
        shareholdingratio: fieldsValue.shareholdingratio?Number(fieldsValue.shareholdingratio):null,//持股比例
        couponrate:fieldsValue.couponrate?Number(fieldsValue.couponrate):null, //折扣率
        yieldtomaturity:fieldsValue.yieldtomaturity?Number(fieldsValue.yieldtomaturity):null, //到期收益率
        conversionprice:fieldsValue.conversionprice?Number(fieldsValue.conversionprice):null, //转换价格
        ipoexchange:fieldsValue.ipoexchange?Number(fieldsValue.ipoexchange):null, //发行平台
        www: fieldsValue.www, //网站网址
        briefdesc: fieldsValue.briefdesc, //简介
        memo: fieldsValue.memo, //备注
        tag:fieldsValue.tag, //标签
        tokentype:fieldsValue.tokentype, //代币类型
        hardcap: fieldsValue.hardcap?Number(fieldsValue.hardcap).toFixed(4):null,  //硬顶
        softcap: fieldsValue.softcap?Number(fieldsValue.softcap):null,  //软顶
        totalsupply: fieldsValue.totalsupply?Number(fieldsValue.totalsupply):null, //总供给
        forsalerate: fieldsValue.forsalerate?Number(fieldsValue.forsalerate).toFixed(4):null,  //可售比率
        lockup: fieldsValue.lockup?Number(fieldsValue.lockup):null, //锁定数量
        valuation: fieldsValue.valuation?Number(fieldsValue.valuation).toFixed(4):null, //本轮估值
        prevaluation:fieldsValue.prevaluation? Number(fieldsValue.prevaluation):null, //上轮估值
        premium:fieldsValue.premium?Number(fieldsValue.premium).toFixed(4):null, //本轮估值溢价
        forsale:fieldsValue.forsale?Number(fieldsValue.forsale):null, //可售额
        price: fieldsValue.price?Number(fieldsValue.price):null,  //价格
        discount: fieldsValue.discount?Number(fieldsValue.discount):null, //折扣
        preinvestor:fieldsValue.preinvestor,  //上轮投资人(逗号分隔)
        investor: fieldsValue.investor, //本轮投资人(逗号分隔)
        otherinvestors:fieldsValue.otherinvestors,  //其他投资人(逗号分隔)
        project_status:this.state.project_status //项目状态
      }
      if(this.state.id){
        investplan.id = this.state.id;
      }

      const val = fieldsValue.region;
      const aa = {};
      var pro = "";
      var cit = "";
      var reg = "";
      ...
      const obj ={
        reqData:{
          ...fieldsValue,
          // country:fieldsValue.country[0],
          corpdesc:this.state.editorState.toHTML(),
          roadmap:this.state.editorState1.toHTML(),
          hotcommunity:this.state.editorState2.toHTML(),
          ...
          /*istoken: tt,*/
          lp_id:this.state.lp_id,
          investplan,
        }
      };
      console.log("obj",obj)
      dispatch({
        type: 'fundproject/ongoingupdateproject',
        payload: obj,
        callback:(res)=>{
          console.log("测试",res);
  ...
  onChangeistoken = (e) =>{
    this.setState({istoken:e.target.checked})
  };
      ...
      <Row>
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.stocktype}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.investtool}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.pricepershare}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.parvalue}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.noofcapital}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.shareholdingratio}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.couponrate}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.yieldtomaturity}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.conversionprice}>
  ...
  renderToken(getFieldDecorator){
    return (
      <Row>
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.ipoexchange}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.tokentype}>
          ...
          <Col  xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.hardcap}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.softcap}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.totalsupply}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.forsalerate}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.Lockup}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.valuation}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.prevaluation}>
          ...
          <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.premium}>
        ...
        <Row gutter={16}>
          <Col lg={6} md={12} sm={24}>
            <Form.Item label={fieldLabe.forsale}>
          ...
          <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
            <Form.Item label={fieldLabe.price}>
          ...
          <Col  xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.discount}>
        ...
        <Row gutter={16}>
          <Col lg={12} md={12} sm={24}>
            <Form.Item label={fieldLabe.briefdesc}>
        ...
        <Row gutter={16}>
          <Col xl={{ span: 24 }} lg={{ span: 24 }} md={{ span: 24 }} sm={24}>
            <Form.Item label={fieldLabe.memo}>
  ...
  render() {
    const {
      fundproject: { datasourced,datasources },
      loading,
    } = this.props;
    //const columdata = this.props.location.state.query.query.attachmentList;
    const { uploading, fileList,fileListend } = this.state;
    ...
    const propsed = {
      onRemove: (file) => {
        this.setState((state) => {
          const index = state.fileListend.indexOf(file);
          const newFileList = state.fileListend.slice();
          newFileList.splice(index, 1);
          return {
            fileListend: newFileList,
          };
        });
      },
      beforeUpload: (file) => {
        this.setState(state => ({
          fileListend: [...state.fileListend, file],
        }));
        /*dispatch({
          type: 'formManage/beforeupload',
          payload: file,
    ...
    const {projectFinanceList,
      projectShareholderList,
      projectHistoryList,
      projectDistributionList,
      projectTeamList} = this.state;

    this.setState({
      aa:data.roadmap,//路线图
      bb:data.corpdesc,//公司简介
      hotcommunity:data.hotcommunity,//社区热度
    });

    const { width, editorState, editorState1, editorState2,financeTableData,dataSource } = this.state;

    const options = this.state.dataList.map(group => (
      <Option key={group.id}>{group.name}</Option>
    ));
    const optionValue = (id)=>{
      const value = this.state.personList.find(item=>{
    ...
    const option = this.state.dataList.map(group => (
      <Option key={group.id}>{group.name}</Option>
    ));
        ...
        <Row>
          <Col span={22}>
            <Form layout="vertical" hideRequiredMark >
              <Card title="完善信息" className={styles.card} bordered={false}>
                <Row gutter={16}>
                  <Col lg={6} md={12} sm={24}>
                    <Form.Item label={fieldLabels.project_name}>
                  ...
                  <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
                    <Form.Item label={fieldLabels.maininvestor}>
                      {getFieldDecorator('maininvestor', {
                        initialValue:data.maininvestor?optionValue(data.maininvestor):'',
                  ...
                  <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
                    <Form.Item label={fieldLabels.createdate}>
                      {getFieldDecorator('createdate', {
                        initialValue:data.createdate?momentt(data.createdate):'',
                  ...
                  <Col lg={{ span: 24 }} md={{ span: 24 }} sm={24}>
                    <Form.Item label={fieldLabels.region}>
                      {getFieldDecorator('region', {
                        ...
                        <Cascader
                          // options={this.state.cityed?addressOptions:''}
                          options={addressOptions}
                          placeholder="请选择地区"
                          // disabled={!this.state.cityed}
                ...
                <Row gutter={16}>
                  ...
                  <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
                    <Form.Item label={fieldLabels.contactmp}>
                      ...
                      })(<Input placeholder="请输入联系人电话" type='number'/>)}
                    </Form.Item>
                  </Col>
                </Row>
                <Row gutter={16}>
                  {/*<Col lg={6} md={12} sm={24}>
                    ...
                    <Form.Item label=''>
                      {getFieldDecorator('istoken',{
                        ...
                        <Checkbox onChange={(e)=>this.onChangeistoken(e)} checked={this.state.istoken}>是否涉及Token</Checkbox>
                      )}
                    </Form.Item>
                  </Col>*/}
                  <Col  lg={{ span: 24 }} md={{ span: 24 }} sm={24}>
                  ...
                  <Col lg={24} md={24} sm={24}>
                    <Form.Item label='上市代码'>
                      {getFieldDecorator('ticker', {
                        initialValue:data.ticker,
                      })(<Input placeholder="请输入上市代码" disabled={!this.state.istoken}/>)}
                    </Form.Item>
                  </Col>
                </Row>
                <Row gutter={16}>
              ...
              <Card title="投资计划" bordered={false} id='11'>
                <Form
                ...
                >
                  <Row>
                    <Col lg={6} md={12} sm={24}>
                      <Form.Item label={fieldLabe.investtype}>
                  ...
                  <Card title="共同" className={styles.card} bordered={false} >
                    ...
                    <Row gutter={16}>
                      <Col lg={6} md={12} sm={24}>
                        <Form.Item label={fieldLabe.subscriptionamount}>
                      ...
                      <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
                        <Form.Item label={fieldLabe.noofshare}>
                      ...
                      <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
                        <Form.Item label={fieldLabe.currency}>
                          {getFieldDecorator('currency', {
                    ...
                    <Row gutter={16}>
                      <Col lg={6} md={12} sm={24}>
                        <Form.Item label={fieldLabe.preinvestor}>
                      ...
                      <Col xl={{ span: 6, offset: 2 }} lg={{ span: 8 }} md={{ span: 12 }} sm={24}>
                        <Form.Item label={fieldLabe.investor}>
                      ...
                      <Col xl={{ span: 8, offset: 2 }} lg={{ span: 10 }} md={{ span: 24 }} sm={24}>
                        <Form.Item label={fieldLabe.otherinvestors}>
              ...
              <Card title="公司简介"  style={{marginTop:'25px'}} bordered={false} id='2'>
                <BraftEditor
              ...
              <Card title="路线图" className={styles.card} style={{marginTop:'25px'}} bordered={false} id='3'>
                <BraftEditor
                ...
                {/*     {getFieldDecorator('roadmap',{
                  initialValue: BraftEditor.createEditorState(data.roadmap),
                })(
                  <BraftEditor
                    value = 'llll啦啦啦'
                    value={this.state.editorState1}
              ...
              <Card title="社区热度" style={{marginTop:'25px'}} className={styles.card} bordered={false} id='4'>
                <BraftEditor
              ...
              <Card title="公司财务" style={{marginTop:'25px'}} className={styles.card} bordered={false} id='5'>
                {getFieldDecorator('finance', {
                  initialValue: projectFinanceList,
                })(<FinanceTableForm/>)}
              </Card>

              <Card title="股东结构" style={{marginTop:'25px'}} bordered={false} id='6'>
                {getFieldDecorator('shareholder', {
                  initialValue: projectShareholderList,
                })(<ShareholderTableForm />)}
              </Card>

              <Card title="融资历史" style={{marginTop:'25px'}}  bordered={false} id='7'>
                {getFieldDecorator('history', {
                  initialValue: projectHistoryList,
                })(<FinancingTableForm />)}
              </Card>
              ...
              <Card title="分配方案" style={{marginTop:'25px'}} bordered={false} id='9'>
                {getFieldDecorator('distribution', {
                  initialValue: projectDistributionList,
                })(<DistributionPlanTableForm />)}
              </Card>

              <Card title="团队" style={{marginTop:'25px'}} bordered={false} id='10'>
                {getFieldDecorator('team', {
                  initialValue: projectTeamList,
                })(<TeamTableForm />)}
              </Card>

              <Card title="投资者关系" style={{marginTop:'25px'}} bordered={false} id='12'>
                <Row gutter={24}>
              ...
              <Card title="尽职调查" style={{marginTop:'25px'}} bordered={false} id='13'>
                <Row gutter={24}>
                  <Col>
                    <Form onSubmit={this.handleSubmit} hideRequiredMark style={{ marginTop: 8 }}>
                      <FormItem {...formItemLayout} label={<FormattedMessage id="form.title.memo" />}>
                        {getFieldDecorator('askmemo')(
          ...
          <Col span={2}>
            <Anchor>
              ...
              <Link href="#12" title="投资者关系" />
              <Link href="#13" title="尽职调查" />
            </Anchor>
          </Col>
        </Row>
        <FooterToolbar style={{ width }}>
          <Button onClick={(e)=>this.backClick(e)}>返回</Button>
          {
            this.state.showSubmit?<Button type="primary" onClick={()=>this.validate()} loading={loading}>
              提交
            </Button>:''
          }
        </FooterToolbar>
...
export default OngoingProjectUpdateProjectForms;
