...
import { Table, Button, Tooltip,AutoComplete,Input, Form,message,DatePicker, Popconfirm, Divider } from 'antd';
...
@Form.create()
class MarkCadd extends PureComponent {
  index = 0;
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.onMark.markstatues !== this.props.onMark.markstatues){
      this.setState({
        vStatus:nextProps.onMark.markstatues
      })
    }
  }
  ...
  render() {
    const {
      dispatch,
      onMark:{onOk,onFocus,onCancle,markstatues},
    } = this.props;
    ...
    return (
      <Fragment>
        ...
        <div style={{display:'flex',justifyContent:'flex-end',margin:'20px'}}>
          <Button onClick={()=>this.cancelLick(onCancle)}  disabled={markstatues}>
            取消
          </Button>
          <Button type='primary' style={{marginLeft:'20px'}} onClick={()=>this.validate(onOk)}  disabled={markstatues}>
            确定
          </Button>
        </div>
      </Fragment>
